...
Dato un linguaggio regolare $L$ è possibile dimostrare che $\neg L\in REG?$

**Dimostrazione:**
Sia $M$ un automa che accetta il linguaggio $L$. Dobbiamo quindi costruire un automa $M'$ che accetta il linguaggio $\neg L$. 

Per fare ciò, prendiamo l'automa $M$ e scambiamo gli stati accettanti con quelli non accettanti.  Bisogna quindi verificare che l'automa $M'$ accetti il linguaggio $\neg L$. 

Notiamo innanzitutto che $M$ ed $M'$ sono dei DFA, pertanto essi hanno tutte le transizioni definite per ogni stato. Di conseguenza, alla fine della computazione i DFA si troveranno in uno degli stati che li compongono.

- **$\implies:$** Se $w\in \neg L$, allora $M'$ accetta $w$.
Sia $w$ una stringa appartenente ad $\neg L$. Vogliamo verificare che l'automa $M'$ accetta $w$. Notiamo che
$$w\in \neg L \implies w\notin L\implies M \text{ non accetta }w.$$
Perciò, la stringa  lascia l'automa $M$ in uno stato non accettante, ma gli stati che non sono accettanti in $M$ sono finali in $M'$. Quindi $M'$ accetta $w$.
- $\Longleftarrow:$ Se $M'$ accetta $w$, allora $w\in \neg L$.
Per semplicità, dimostriamo che se $w\notin \neg L,$ allora $M'$ non accetta $w$.

Sia $w$ una stringa non appartenente a $\neg L,$ quindi $w\in L.$ Questo vuol dire che $w$ è accettata dall'automa $M$, ma tutti gli stati che sono finali in $M$ sono stati non accettanti in $M'$, pertanto $M'$ non accetta $w$.
##### Concatenazione
...
- Una computazione **deterministica** prosegue sequenzialmente. Un DFA $M$ accetta/rifiuta a seconda dello stato in cui si trova dopo aver letto l'ultimo simbolo dall'input
- Una computazione **non-deterministica** di un NFA $N$ definisce un albero di computazioni. $M$ rifiuta se tutti i cammini conducono a uno stato non finale. $M$ invece accetta se almeno un cammino dell'albero termina in uno stato finale.
...
- $Q_{D}=\mathcal P(Q_{N})$ - L'insieme degli stati di $D$ corrisponde all'insieme delle parti (ovvero l'insieme di tutti i sottoinsiemi) degli stati di $N$.
  
  Di conseguenza, uno stato di $D$ invece corrisponderà ad un sottoinsieme degli stati di $N$.
- $q_{0}^{D}=\{q_{0}^{N} \}$ - Lo stato iniziale di $D$ è un insieme che contiene lo stato iniziale di $N$ (Perché il suo dominio è l'insieme delle parti)
- $F_{D}=\{R\in Q_{D}:R\cap F_{N}\neq\emptyset \}$ - L'insieme degli stati finali di $D$ corrisponde all'insieme di tutti gli elementi di $Q_{D}$ che contengono almeno uno stato finale di $N$.
- Sia $R\in Q_{D},\ a\in\Sigma:$
$$\delta_{D}(R,a)=\bigcup_{r\in R}\delta_{N}(r,a)$$
Quando $D$ si trova in uno stato $R$, significa che il NFA $N$ simulato da $D$, in quel momento si trova contemporaneamente in tutti gli stati $r\in R.$ Perciò, per definire la $\delta_{D}$ bisogna pensare a cosa succede all'automa $N$ quando legge un input $a$.

Semplicemente, quando l'automa $N$ legge un input $a$, per ciascuno dei suoi stati correnti, deve guardare quali sono gli stati raggiungibili attraverso $a$.

In sintesi, $\delta_{D}(R,a)$ è definito come l'unione di tutti gli stati raggiungibili tramite un input $a$ a partire da uno stato $r\in R$.
...
- **Caso generale:** Ci sono $\mathcal{E}-\text{archi}$. 
Bisogna quindi tenere traccia degli sati raggiungibili usando $\mathcal{E}-\text{archi}$.

Sia $R\in Q_{D}.$ 
Definisco $E(R)=\{q\in Q_{N}: q$ può essere raggiunto da stati in $R$ attraverso $\ge0$ $\mathcal{E}-\text{archi in } N\}$. 
A questo punto: $$\delta_{D}(R,a)=\bigcup_{r\in R}E(\delta_{N}(r,a))$$
Quando l'automa $D$ si trova in uno stato $R$, l'automa $N$ non si trova solo negli stati contenuti in $R$, ma in tutti quegli stati che sono raggiungibili a partire da uno stato $r\in R$ attraverso un numero $\ge0$ di $\mathcal{E}-$archi.

Bisogna infine modificare la definizione di $q_{0}^{D}$ in:
$$q_{0}^{D}=E(\{q_{0}^{N} \})$$
Perché lo stato iniziale di $D$ deve tenere conto di tutti gli stati raggiungibili in $N$ da $q_{0}$ mediante solo $\mathcal{E}-$archi.

Esercizio: Dato il seguente $NFA$
![[lez03_04.drawio.svg|700x505]]
Ricavare il $DFA$ $D$ tale che $L(D)=L(N)$.
<font color="#000000"><span style="background:#b1ffff">Svolgiamolo!</span></font>

![[lez03_05.drawio.svg|700x453]]
