...
Un Automa a Stati Finiti (Abbreviato con **DFA** -  Deterministic Finite Automata) è una quintupla: $$(Q,\Sigma,\delta,q_{0},F)$$dove:
...
| $$q_3$$ | $$q_2$$ | $$q_2$$ |
>[!info] Nota:
Notiamo che, essendo $\delta$ una funzione definita come
$$\delta:Q\times\Sigma\longrightarrow Q$$
un DFA deve avere per ogni stato esattamente una freccia di transizione per ogni possibile simbolo di input.
...
$$\delta^{*}:Q\times\Sigma^{*}\longrightarrow Q$$
(dove $\Sigma^{*}$ è l'insieme di tutte le stringhe sull'alfabeto $S$, inclusa la stringa vuota $\mathcal{E}$).
...
Un automa $M=(Q,\sigma,\delta,q_{0},F)$ accetta una stringa $w\in\Sigma^{*}$ se e solo se $$\delta^{*}(q_{0},w)\in F.$$
Il linguaggio riconosciuto da $M$ è quindi: $L(M)=\{w\in\Sigma^{*},\delta^{*}(q_{0},w)\in F\}$. In altre parole, $L(M)$ è l'insieme di tutte le stringhe che, partendo dallo stato iniziale $q_{0}$ portano l'automa in uno degli stati di accettazione in $F.$

>[!info] Nota:
> Il linguaggio può anche essere $L(M)=\emptyset.$
...
3. Passo di computazione $\longrightarrow$ il passaggio da una configurazione ad un'altra rispetto ad una relazione binaria $\vdash_{M}$ definita come:$$\begin{aligned}(p,ax)\underset{\substack{\text{in relazione}\\ \text{secondo } M\  \text{che}\\ \text{è il DFA fissato}}}{\underbrace{\vdash_{M}}}(q,x)\iff \begin{cases} p,q\in Q \\ a\in\Sigma,x\in\Sigma^{*}\\ \delta(p,a)=q \end{cases}\end{aligned}$$
...
Gli passo la stringa $011$. L'automa interpreta la stringa come segue:$$\begin{aligned}(q_{1}, 011)&\implies \vdash_M (q_{1}, 11)\\ &\implies \vdash_M (q_2, 1)\\ &\implies \vdash_M (q_2, \varepsilon)\\ \mathcal{E}&=\text{Stringa Vuota}\end{aligned}$$ Quindi:$$(q_1, 011) \vdash_M^* (q_2, \varepsilon)$$ che è equivalente al dire che:$$\delta^*(q_1, 011) = (q_2, \varepsilon)$$
Essendo $q_{2} \in F$, vuol dire che la stringa $011$ è accettata dall'automa, quindi: $w=011 \in L(M).$
